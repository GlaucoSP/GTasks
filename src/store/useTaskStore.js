...
const useTaskStore = create((set, get) => ({
  ...
  updateList: (listId, updatedList) => set((state) => ({
  ...
  updateTask: (listId, taskId, updatedTask) => set((state) => ({
    lists: state.lists.map(list =>
      list.id === listId
        ? {
            ...list,
  ...
  deleteCompletedTask: (taskId) => set((state) => ({
    completedTasks: state.completedTasks.filter(t => t.id !== taskId)
  })),
  ...
  loadData: async () => {
    try {
      ...
      const lists = listsData[1] ? JSON.parse(listsData[1]) : [{
        id: 'default',
        title: 'Padrão',
        color: '#000000',
        bgColor: '#f0f0f0',
